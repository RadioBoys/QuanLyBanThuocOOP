...
public class RegexExpression {
    ...
    public static ArrayList   search(String text) throws IOException
	{
		String REGEX_ROOM =".*"+text+".*";
		Pattern pattern =Pattern.compile(REGEX_ROOM);
		Matcher matcher;
		List_Thuoc dsthuoc=new List_Thuoc();
		dsthuoc.docFile();
		ArrayList<Thuoc> thuocsearch=new ArrayList();
		for(Thuoc i:dsthuoc.l_thuoc)
		{
			matcher=pattern.matcher(i.getTenthuoc());
			if(matcher.find())
			{
				thuocsearch.add(i);
			}
